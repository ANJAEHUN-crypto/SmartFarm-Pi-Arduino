...
## 13차 배포 (릴레이 안정화 + 센서 3초 주기 복원)

- **1. 릴레이 안정화**: 릴레이 상태 조회 주기를 **30초** 수준으로 낮추고, 시리얼 명령 직렬화·응답 대기 확대·자동 재연결/재시도를 추가해 릴레이 통신 안정성 강화.
- **2. 스케줄 보강**: 스케줄 실행 시 `is_open()` 조건에 막혀 건너뛰지 않도록 하여, 내부 재연결 로직을 통해 릴레이 명령이 복구 경로를 타게 조정.
- **3. 센서 주기 복원**: 아두이노 센서 송신 주기를 **3초**로 복원하고, RS485 응답 대기 중에도 USB 시리얼 명령을 처리하도록 개선.
- **삭제**: 없음.
- **상세**: `patch/13.md` 참고.

---

## 14차 배포 (센서 오류값 차단 + 30초 주기 완화)

- **1. 센서 오류값 차단**: Modbus 응답(CRC/프레임) 검증과 센서 범위 검증을 추가해 `65535`, `6553.5`, `-1` 계열 값이 반영되지 않도록 강화.
- **2. 저장/전송 안전화**: Pi에서 센서 payload를 재검증해 비정상 프레임은 저장/퍼블리시에서 제외.
- **3. 조회/표시 완화**: 웹 현재값은 최근 샘플 중앙값으로 계산하고, 배지 조회 주기를 **30초**로 완화.
- **4. 센서 주기 조정**: 아두이노 센서 송신 주기를 **30초**로 조정해 RS485 통신 부하 완화.
- **삭제**: 없음.
- **상세**: `patch/14.md` 참고.
