...
- **4. 웹 UI**
  ...
  - **즉시 촬영·업로드 버튼**: 릴레이 원격 ON/OFF 보드와 비슷한 스타일로, 주기와 관계없이 1회 촬영 후 드라이브 업로드 실행 (`POST /api/camera/capture-once`).
...
## 12차 배포 (카메라 시간별 노출 설정 + UI 통일감 개선)

- **1. 카메라 설정 개편**: 웹 노출 설정을 `낮/저녁/야간` 3구간에서 **00~23시 시간별 설정**으로 전환. 기존 포맷(`day/evening/night`)은 읽을 때 자동 호환.
- **2. 웹 UI 정리**: 릴레이/스케줄/카메라 카드의 가로 폭, 폰트, 버튼 높이, 둥근 모서리, 배경 톤을 맞춰 전체 화면 통일감 강화.
- **삭제**: 없음.
- **상세**: `patch/12.md` 참고.
